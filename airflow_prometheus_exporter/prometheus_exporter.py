    ...
    with session_scope(Session) as session:
        ...
        dags = (
            session.query(
                ...
                DelayAlertMetaData.alert_external_classification,
                DelayAlertMetaData.alert_report_classification,
                DelayAlertMetaData.confluence_page,
        ...
        for dag in dags:
            dag_metrics = {
                ...
                "confluence_page": dag.confluence_page or "",
    ...
    with session_scope(Session) as session:
        ...
        for task in tasks:
            task_metrics = {
                ...
                "confluence_page": task.confluence_page or "",
...
class MetricsCollector(object):
    ...
    def collect(self):
        ...
        for task in get_sla_miss_tasks():
            sla_miss_tasks_metric.add_metric(
                [
                    task["dag_id"],
                    task["task_id"],
                    task["alert_target"],
                    task["alert_external_classification"],
                    task["alert_report_classification"],
                    task["severity"],
                    task["confluence_page"],
                    task["group_pagerduty"],
                    task["group_business_line"],
                    task["inhibit_rule"],
                ],
                task["sla_miss"],
            )
